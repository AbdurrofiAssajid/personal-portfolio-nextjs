...
export default function Header() {
  ...
  <nav className="flex fixed top-[0.15rem] left-1/2 h-12 -translate-x-1/2 py-2 sm:top-[1.7rem] sm:h-[initial] sm:py-0">
    <ul className="flex w-[30rem] flex-wrap items-center justify-center gap-y-1 text-[0.9rem] font-medium text-gray-600 dark:text-gray-300 sm:w-[initial] sm:flex-nowrap sm:gap-5">
      {links.map((link) => (
        <motion.li
        ...
        >
              <Link
                className={clsx(
                  "flex w-full items-center justify-center px-3 py-3 hover:text-blue-800 transition dark:text-white dark:hover:text-yellow-400",
                  {
